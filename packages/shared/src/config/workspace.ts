...
export const workspaceDeclarationSchema = z
  .object({
    ...
    contextDirs: z
      ...
      .max(64)
      .describe(
        "Additional directories, relative to the primary root or absolute, that join the workspace's working set for context and eventually tool scope; exists because J-D1 defines a workspace as a declared working set in the VS Code sense — a primary root plus extra directories. Entries that do not exist are skipped with a warning, never a failure.",
      )
      .optional(),
  })
  .strict()
  .describe(
    "Workspace declaration at `.keywork/workspace.json`; its parent of `.keywork/` is the primary root. Discovered by walking from the launch directory up to the filesystem root, nearest declaration wins (git-style), so keywork opens the workspace from any subdirectory. Workspace identity keys off the resolved primary root; the workspace-scope memory vault lives at `.keywork/memory/` beside this file, in-repo and git-able.",
  );
...
function findDeclarationAbove(dir: string): string | undefined {
  const candidate = join(dir, ".keywork", "workspace.json");
