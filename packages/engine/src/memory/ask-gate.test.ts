...
const scratch = scratchDirs("keywork-askgate-");
...
describe("AskGateLedger", () => {
  it("turns three same-shape approvals into exactly one staged proposal and a preference note", async () => {
  ...
  it("never proposes the same shape twice, even after the item is resolved", async () => {
    ...
    const [first] = await ledger.proposePreferences(store);
    await store.discard(first?.id ?? "");
    await ledger.record("bash git", "yes");

    expect(await ledger.proposePreferences(store)).toHaveLength(0);
    expect(await store.listStaged()).toHaveLength(0);
  });
  ...
  it("resets the streak when an ask is denied", async () => {
    ...
    await ledger.record("bash git", "yes");
    expect(await ledger.proposePreferences(store)).toHaveLength(1);
  });
  ...
  it("persists events and proposal fingerprints across restarts", async () => {
    ...
    await first.proposePreferences(await openVault());

    const reopened = new AskGateLedger({ filePath: file });
    expect(await reopened.events()).toHaveLength(3);
    expect(await reopened.proposePreferences(await openVault())).toHaveLength(0);
  });
});
