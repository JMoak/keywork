...
contrast against the pane ground (C17 tokens); reduced-motion honored, so animations
...
cyan, Tokyo Night natives, interpolated perceptually in OKLCH) and every hued surface
draws from it:
...
| masthead | <40 | none | none | none | transcript yields to a fitted headline: session topic in block glyphs (tier-2; tier-0 caps fallback) + one status line |
...
(closest to you) · `▓` agent · `░` machine output. It is the same "denser = closer to you"
...
outcome, and outcome is the only colored word. Expanded detail hangs under a faint rule;
...
- **Needs-you chrome** (added 2026-08-21, 104/PD25): the pane answers its stamp.
  ...
  `background`), which is the selection/cursor inversion's existing meaning, *the thing
  your keystroke targets*, applied to a pane; and the border takes its identity hue with a
