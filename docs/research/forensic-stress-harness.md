...
> `docs/backlog/archive/92-iteration-3.md` (MCP status), `docs/backlog/archive/93-adversarial-review.md` (WP-1/WP-2
...
| 5 | `app.ts` `mcpDropWatcher` | Subscription never unsubscribed; its `lastStates` map is never pruned, so it is unbounded under server-name churn. | **fixed** (subscription torn down in `onExit`, plus a render closed-guard behind it); `lastStates` pruning moot while the server set is config-static per process. |
...
work that closed #4/#5; see `backlog/archive/92-iteration-3.md` "MCP lifecycle hardening". The harness
...
4. **Handle-count trend (secondary, trend-only):** `(Get-Process -Id $pid).HandleCount` sampled
   per cycle. Noisy by nature (GC, threadpool), so flag only monotonic growth across ≥5 consecutive
