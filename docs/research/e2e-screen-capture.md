...
`keypress` listener `runApp` registers, so synthetic input exercises the genuine chord
...
2. `onExit` (`app.ts:208-219`) calls `process.exit(0)` after `renderer.destroy()`, so a
...
screencast once tier 1 exists, since the span recorder's `TestRecorder` frame stream is the
natural input to it).
