...
> **Status of this document.** Sections 1 to 3 (philosophy, the shared-features baseline,
> per-tool adoption verdicts) are the distilled position of the research phase and remain the
> evidentiary base. **Section 4's ten open questions (Q1–Q10) are resolved by
> [`vision.md`](vision.md) D1–D10**; the "Decision" leans written under each question here
> are research history, and where they and `vision.md` disagree (MCP posture, Q1 vs D1, is
> the clearest case) the decision wins. Task ids live in [`backlog/`](backlog/README.md).
>
> **Update (2026-08-10):** Crush is no longer a design source for keywork. Crush columns and
...
> - **Pi** ([earendil-works/pi](https://github.com/earendil-works/pi)) is **MIT**: code may be lifted with attribution.
> - **OpenCode** ([sst/opencode](https://github.com/sst/opencode)) is **MIT**: code may be lifted with attribution.
> - **Crush** ([charmbracelet/crush](https://github.com/charmbracelet/crush)) is **FSL-1.1-MIT**: **ideas only, never copy its source.** Where Crush and an MIT tool share a mechanic, study/lift the MIT implementation instead.
...
|---|---|---|---|
| Tree-structured JSONL sessions (`id`/`parentId`, `/tree`, `/fork`, `/clone`, labels, branch summaries) | **Yes** | Crown jewel; maps directly onto multi-pane workflows, with panes as leaves of one tree | **Lift** the documented session format |
...
| No built-in permission system at all | **No** | Too raw for keywork's taste; ship a thin default gate extension instead | n/a |
...
| Share links (`opncd.ai`) + cloud service | **No** | Out of scope for a keyboard-first harness; requires running a service | n/a |
| Electron desktop app, web UI, Slack/enterprise packages | **No** | Not keywork's surface; the server shape keeps the door open | n/a |
...
| Feature | Verdict | Why | License note |
|---|---|---|---|
| LSP-powered agent context (real language servers as agent tools) | **Later** | Highest-leverage Crush idea for a TypeScript-first harness (`typescript-language-server`/`vtsls`); heavy lifecycle cost, so after core | **Reimplement** from scratch; or lift OpenCode's MIT LSP layer instead |
| Live shared workspaces (same `--cwd` ⇒ implicit join, live session mirroring) | **Yes** (as target) | Direct hit on keywork's multi-window/pane value; keywork's server/event architecture should be designed so this falls out | **Reimplement** with its own protocol (local socket/Bun IPC + storage) |
...
| MCP over stdio/http/sse with OAuth | **No** (in core) | Conflicts with the chosen Pi posture (§4 Q1); an extension can bridge later | n/a |
| Bash-based `crushrc` config | **No** | Clever, but typed TS/JSON config with schema validation fits keywork's simplicity value better | n/a |
| Very broad provider matrix; Android/BSD targets | **No** (for now) | Scope discipline: few providers/platforms done excellently first | n/a |
...
default* and candidly recommends plain CLI linters/typecheckers; Pi has no LSP at all, just bash
