...
| [`events.md`](events.md) | **Event vocabulary of record** (A5, 2026-09-07): the envelope, the fifteen bus event types with their payload fields and when each fires, the SSE framing `keywork serve` uses, the `Last-Event-ID` resume contract, and the one wire translation (`engine.error`). `packages/engine/src/bus.test.ts` reads it and fails when a heading and `EngineEvents` disagree. |
| [`compliance/anthropic-review.md`](compliance/anthropic-review.md) | **G2 guardrail review of record** (2026-09-03): the checklist run against the Anthropic provider diff (no OAuth paths, no subscription endpoints, no client impersonation, key never logged, API-key-only policy stated to users), each box with its grep and test evidence, plus Jordan's sign-off line. Re-run on every change under `engine/src/providers/anthropic.ts`. |
| [`live-smoke/anthropic.md`](live-smoke/anthropic.md) | The manual live check for the Anthropic provider with a real key: connect, model switch, tool turn, cache tokens on the cost line, and the red flags that mean stop. |
