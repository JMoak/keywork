...
> [`94`](94-file-browser-and-mouse.md) (each within its scope) → [`92`](archive/92-iteration-3.md) →
> [`91`](archive/91-progress-and-feedback.md) → [`90`](archive/90-plan-review.md) → workstream
> files apply (90 to 92 are archived under `archive/`).
>
...
budgets (R4), the vault layout (J3), and the airlock, so the cost of a wrong cut is high.
...
(`Math.max(10, width - 4)`, `padEnd(width)`) at sizes the drawn box may not have, so
...
2. Must a linked folder be independently trusted (E6 nearest-ancestor) before it widens
   ...
   linking grants scope; trust gates activation; an untrusted linked dir stays inert.)
