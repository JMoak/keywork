...
> Authoritative overlay, 2026-08-15, from Jordan's third vision pass. PD8, PD9, PD10 and the
> addendum below bind as written, with two later refinements: [`99`](99-workspace-and-modes.md)
> refines PD10 (focus dirs, per-subpath MRU, nested anchors rejected) and supersedes the
> split-into-mode clause; [`110`](110-arcs-on-screen.md) records C45, C46 and J19 as landed.
> Where this file is silent, [`97`](97-product-direction.md) and the workstream files apply;
> the older overlays (90 to 92) are archived under `archive/`.
>
...
> FSL and never a source (no code, no design credits since 2026-08-10). The user commits;
...
4. **Arcs claim anchors.** When arcs exist (J17), each arc takes an anchor hue on the
   ...
   surface also carries the arc's slug tag, so under `NO_COLOR` the grouping stays
   legible as text.
...
"subset" point) but remain explicitly searchable; archived arcs are excluded, since their
...
(idea 4), where distillation writes one workspace-layer cover-sheet note per arc (entities
...
(idea 5), where the digest triages the arc's open-question notes as resolve / carry forward /
...
- design-language.md gains the chroma section (hue = identity/depth, never state);
  the same document remains the vocabulary of record.
