...
- **C52 (2pt): fence highlighting** ✅ 2026-08-21: `tui/highlighter.ts`, a zero-dep
  lexical tokenizer written as grammar *data* over one scanner, covering ts/js/tsx/jsx, json
  (keys vs values), sh/bash/zsh (`$VAR`, boundary-only `#`), py (triple quotes), go, rust,
  diff (line marks), and md, with per-fence block state so block comments, template
...
5. **The away-state stays G6's.** This addendum is the return-state made visible in the
   ...
   `triggerNotification` / OSC 777 / bell) remains G6 (archive/90-plan-review), decoupled.
