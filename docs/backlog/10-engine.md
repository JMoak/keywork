...
`AGENTS.md` / project instructions when present; measured, with a target within ~15% of Pi's
token count, tracked in a test.
...
error messages are UX, so make non-unique / not-found failures instructive for the model.
