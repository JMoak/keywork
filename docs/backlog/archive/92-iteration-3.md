...
> **Archived 2026-08-22.** Superseded by `97-product-direction.md` and the later overlays; kept
> as the iteration-3 ledger. Its rules no longer bind (the platform priority below was carried
> forward by the later overlays).
...
| (none) | extra | `AppCore` extraction (pure state machine, the external-agent event surface), global `keywork` shim, 0600 key storage. |
...
| V2 outline | drafted (2026-08-10) | `96-conversation-enrichment.md`, the streaming-feed/conversation-pane enrichment overlay: 12 sized candidates (tool tail-follow, diff-preview ask, markdown, thinking blocks, context/cost HUD, queue editing, @-mentions, `!` escape, recall citations, retrieval disclosure, provenance gutter, OSC) bound to design-language principles; unadopted until the coding-agent nuance survey (`docs/research/coding-agent-nuances.md`, in flight) merges. |
...
| J7 kernel | **done** (2026-08-10) | Gardener (`memory/gardener.ts`) + `ReviewInbox` (`memory/inbox.ts`, R3's one inbox at `<vault>/.staging/inbox.json`): score-gated promotion from daily logs behind a structural taint gate (untrusted entries filtered before the `CurationJudgmentPort` sees them; returned proposals re-validated, with tainted/hallucinated/hostile-title refs rejected); merge/contradiction/supersession over Jaccard candidate pairs, auto-action only for agent↔agent above threshold, everything else → inbox proposal; usefulness EMA (α=0.3) with per-session anti-gaming cap, stamped only on agent notes, feeding bootstrap selection (per the fifth-pass F2 resolution it stays OUT of search ranking); unlinked-mention link proposals (never auto-edits); one audit entry per sweep; sweep idempotent; untrusted store inert; human-authored files provably untouchable under a malicious port. Deferred: graph duties → J12, skills telemetry → J10, persistent recall ledger → J13. |
...
| C13 | mostly done (2026-08-10) | Session-tree pane over Track T's `tree()` API: pure `SessionTreeModel` (windowed flattened outline, indent only under branch points, collapse/expand, path-anchored cursor surviving refresh, inline label editor) + `SessionTreePane` (chrome, entry-count title, `●/○` active-path markers, `▸/▾` branch glyphs); `/tree` command + `leader t` summon-or-focus, opens docked (browser precedent); `f` fork → `PaneIntents.openSession` → new session-backed conversation pane (fork attachments pre-opened so history seeds the agent); `session-tree` workspace descriptor persists/revives. Injected `SessionTreePort` is TUI-side only; the cli `sessionPort`-style disk implementation is the remaining wiring (cli was owned elsewhere this wave). Jump-to-node deferred honestly: switching a live pane's branch requires rebinding the agent's seeded history mid-flight, so it lands with the steer/agent-rebuild seam. Unit + property + probe workflow tests. |
| MCP lifecycle hardening | **done** (2026-08-15) | Registry rewritten as a per-server reconciler: verbs write versioned desired state (contradictory bursts coalesce: restart/disable/restart/enable → one deterministic reconnect, all callers settle at convergence); one `serve()` loop per server owns every connect, close, and backoff sleep, so the loop promise IS the shutdown barrier: `stop()` drains loops to a fixed point and later verbs reject typed `McpRegistryClosedError`; level-triggered `Wakeup` replaces raw retry timers (verbs/stop preempt backoff instantly); actor is total (listener throws quarantined, rejecting `close()` contained, reentrant-listener spawn race guarded). Abortable connects: per-attempt `AbortController` → `connectStdioServer({signal})` (`McpAbortedError`), so disable/stop during a hung handshake tear down in ms instead of waiting out the request timeout. Process containment: `bash.ts` `killTree` lifted to shared `engine/proc.ts`; `StdioChannel.close()` = stdin-EOF + 500 ms grace then VERIFIED tree kill (`taskkill /t` + liveness; POSIX detached spawn + group TERM→KILL); abort = immediate kill; new `leaky` fixture (marker-reporting grandchild, lingers after EOF) pins parent-and-grandchild death after `close()` and `registry.stop()` at both client and registry layers. TUI/CLI disposal hardening (from a full late-completion-after-dispose audit): shared `PaneTasks` gives mcp/memory/session-tree panes dispose semantics (no new work, gated notify, `settled()` still drains); `runApp` render closed-guard + armed-timer clear + drop-watcher unsubscribe in `onExit` (killed a render-into-destroyed-renderer use-after-free window); `startFreshSession` liveness check + single-use attachments (no session attach to a closed pane); `AppCore.onPaneClosed` prunes per-pane registries; conversation-model disposed guards (confirm auto-denies, submit/disclose inert, fork/title late results dropped) while the in-flight turn's after-turn persistence deliberately survives close; workspace `flush()` → terminal `seal()`; MCP pane holds conflicting actions while a server transition is in flight (engine serialization stays the correctness boundary). Fixed en route: `listTools`-failure connection leak, close-error masking the handshake cause. Adversarially re-passed twice (multi-agent review + fixes with regression tests each time). `research/forensic-stress-harness.md` §2 re-statused (6 of 10 defects closed, #8 semantics now decided-by-implementation), §9 item 1 done; harness itself still unbuilt. Gate: 1089 tests / 76 files, checks green. |
