...
> **Archived 2026-08-22.** All eight work packages landed 2026-08-10; kept as the fixed-defect
> corpus and the review-wave precedent. Its sequencing no longer binds.
...
tui: palette runs arg-requiring commands with no args/no affordance (F7); help overlay non-modal (F8); pane dims use raw terminal size ignoring border+status, dock percent re-rounded (F9); no resize handler (F10); Unicode as UTF-16 units, so emoji cannot be typed and wrap shreds surrogate pairs/CJK (F11); `chordOf` ignores kitty `option` so alt-bindings dead on kitty (F12); palette selection diverges on async retitle (F13); `keywork run` with no provider fabricates a fake successful response incl. `--json` (F16).
