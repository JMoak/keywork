...
> as a record of the 2026-08-09 review. Its rules no longer bind.
...
- **C15 split (was 3pt, dishonest):** **C15a (2pt)** is the ConPTY/OpenTUI go/no-go spike +
  fallback read-only agent-bash scrollback pane (this is the M2 demo version); **C15b (8pt,
  M3)** is the full interactive embedded terminal, only if the spike says go.
...
- **A1 acceptance extended:** hand-written Anthropic-*shaped* paper fixtures (thinking blocks,
  cache-token usage) must round-trip the neutral format losslessly; this validates neutrality
  two milestones before G1 wires anything (reading public API docs violates no guardrail).
