...
> scope) → [`92`](archive/92-iteration-3.md) → [`91`](archive/91-progress-and-feedback.md) →
> [`90`](archive/90-plan-review.md) → workstream files (90 to 92 are archived under
> `archive/`).
>
...
> FSL and never a source. The user commits; agents never `git commit`/`git push`.
...
3. **Monorepo shape = root anchor + focus dirs + per-subpath MRU** (the A+C design).
   ...
   overlapping jails and split memory partitions are a semantics swamp, and the in-repo
...
3. **Mode is per-session**, persisted, with this resolution chain:
   ...
   mode → config default (`.describe()`-justified; **ships as Agent**, since the gate
   still asks before mutations). Mode changes are session entries (E5's rule); mode renders
...
1. **Regular split** = new session **in the focused session's arc** (auto-named as
   today); an unbound source yields an unbound session, since arcs never impose themselves.
2. **Split New Arc** = creates a fresh **auto-named** arc (suggestTitle-style,
   ...
   binding (or stays unbound), and pulling the source in is an explicit `/arc switch`.
